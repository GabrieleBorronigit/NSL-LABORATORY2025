...
GA::GA(int N_chromosomes, int N_cities, int rank) {
  int seed[4];
  int p1, p2;
  ifstream Primes("../INPUT/Primes");
  if (Primes.is_open()) {
    for(int i=0;i<=rank;i++){ //every process starts with a different seed
    Primes >> p1 >> p2;}
  } else
    cerr << "PROBLEM: Unable to open Primes" << endl;
  Primes.close();
...
void GA::Random_mutation( Chromosome &c) { // selects a random mutation and performs it
...
//replaces the worst chromosome with the best one from another continent, or eventually at place spots before the end of the population
  void GA:: Replace(const int best[], const mat &D, int place){
    Sort_by_fitness(D);
    int worst=GetN()-1-place;
    for(int i=0;i<_population[0].Get_Ngenes(); i++){
      _population[worst].Set_Gene(i,best[i]);
    }
