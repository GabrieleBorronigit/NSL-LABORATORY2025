...
double compute_chi2(int n, const int M, Random &rnd) {
    double random_number{}; //contiene i numeri random da usare
    vector<int>ni(M,0);     // Array per contare quanti numeri cadono in ogni intervallo
    double chi{};           //risultato

    // Conto quanti numeri random cadono in ogni sottointervallo
    for (int i= 0; i < n; i++) {
    random_number=rnd.Rannyu();
    ni[static_cast<int>(M*random_number)]++; //si puo mostrare che il M*random dia il sottointervallo di appartenenza
    }

    // Eseguo il test del chi-quadrato
    int L = static_cast<int>(n / M); // Valore atteso per ogni intervallo
    for (int j = 0; j < M; j++) {
        chi += pow((ni[j] - L), 2) / L;
    }
