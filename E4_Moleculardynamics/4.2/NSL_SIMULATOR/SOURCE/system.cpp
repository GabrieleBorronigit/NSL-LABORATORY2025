...
void System :: initialize_velocities(){ //serve solo per la MD, anche se non usa esplicitamente le velocità, devo inizializzare quelle iniziali equelle al tempo t0-dt
  ...
  if(_restart){
    ...
    //delta potential
      double v = sqrt(3.0 * _temp); 
 for(int i = 0; i < _npart; i++){
   
//set opposite direction velocities on each axis
    if(i % 6 == 0){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0/(double)_n_bins_v; // vmax=4.0 "empirica"
        _measure_pofv = true;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x, 0)); //ho diviso per 2 per far partire la config da meta del lato per ogni direzione (da togliere)
      _particle(i).setposition(1, this->pbc(_side(1)*y, 1)); //sono ovunque unita di misure naturali, ma la configurazioee iniziale è sempre scritta in udm del lato, per questo moltiplico per la lunghezza del alto stesso.
...
double v=arma::norm(_particle(i).getvelocity()); //bin size= vmax/nbins, in questo momento la vmax è 4T

//find the bin corresponding to v
int bin=(int)(v/_bin_size_v);
//store result if it's valid (in between histogram's bounds)
if(bin<_n_bins_v&& bin>=0)
_measurement(_index_pofv+bin)+=1;
...
if(_measure_pofv){ 
  ...
  coutf  <<setw(12)<<blk //blocco corrente
   <<setw(12)<<(i+0.5)*_bin_size_v //velocità centrale del bin corrente
   <<setw(12)<< _average(_index_pofv+i)
