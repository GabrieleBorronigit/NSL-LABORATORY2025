...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    //probabilità che Si=+1
    double hi=_J*(_particle(pbc(i-1)).getspin()+_particle(pbc(i+1)).getspin())+_H;
    double prob=1/(1+exp(-2.0*_beta*hi));

    //se flippo con probabilità prob
    ...
    _naccepted++; //Gibbs accetta sempre la mossa
...
int System :: pbc(int i){ // Enforce periodic boundary conditions for spins (ising 1D)
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      if(_sim_type > 1){ //simtype=2,3= ising=>salvo i parametri H e J
        input >> _J;
        input >> _H;
      }
      if(_sim_type > 3){
...
void System :: initialize_velocities(){ //serve solo per la MD, anche se non usa esplicitamente le velocità, devo inizializzare quelle iniziali equelle al tempo t0-dt
  ...
  if(_restart){
    ...
    /*for (int i=0; i<_npart; i++){
      ...
      vy(i) = _rnd.Gauss(0.,sqrt(_temp)); //qua devi mettere il potenziale deltiforme
      vz(i) = _rnd.Gauss(0.,sqrt(_temp));
      sumv(0) += vx(i);
      sumv(1) += vy(i);
      sumv(2) += vz(i);
    }*/
    //inizializzo con potenziale deltiforme
 for(int i = 0; i < _npart; i++){
    double v = sqrt(3 * _temp);  
//imposto velocità a due a due opposte su ogni asse
    if(i % 6 == 0){
        vx(i) = +v; vy(i) = 0;  vz(i) = 0;
    } else if(i % 6 == 1){
        vx(i) = -v; vy(i) = 0;  vz(i) = 0;
    } else if(i % 6 == 2){
        vx(i) = 0;  vy(i) = +v; vz(i) = 0;
    } else if(i % 6 == 3){
        vx(i) = 0;  vy(i) = -v; vz(i) = 0;
    } else if(i % 6 == 4){
        vx(i) = 0;  vy(i) = 0;  vz(i) = +v;
    } else if(i % 6 == 5){
        vx(i) = 0;  vy(i) = 0;  vz(i) = -v;
    }
}
...
//non serve riscalare per la deltiforme
/*
    for (int idim=0; idim<_ndim; idim++) sumv(idim) = sumv(idim)/double(_npart);
    double sumv2 = 0.0, scalef;
    for (int i=0; i<_npart; i++){
      vx(i) = vx(i) - sumv(0);
      vy(i) = vy(i) - sumv(1);  //se sommo le gaussiane su tre lati ottengo energia extra, devo sottrarre il "centro di massa " delle velocità. Facendo questo, però, introduco un errore sulla temperatura, che correggo riscalando la velocità
      vz(i) = vz(i) - sumv(2);  //sapendo che v*=sqrt(3T*)
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutt << "#   T     BLOCK:   ACTUAL_TE:    TE_AVE:      ERROR:" << endl;
        coutt.close();
        _nprop++;
        ...
        coutpv << "   #BLOCK      # VELOCITY:     AVE_POFV:        ERROR:" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.5/(double)_n_bins_v; // vmax=4.5 "empirica"
        _measure_pofv = true;
        _index_pofv = index_property;
        index_property += _n_bins_v;
      } else if( property == "MAGNETIZATION" ){
        ofstream coutpr("../OUTPUT/magnetization.dat");
        coutpr << "#      T        BLOCK:   ACTUAL_M:     M_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        coutpr << "#       T       BLOCK:   ACTUAL_CV:    CV_AVE:      ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        coutpr << "#      T         BLOCK:   ACTUAL_X:     X_AVE:       ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
...
void System :: write_configuration(){
  ...
  if(_sim_type < 2){
    ...
    if(coutf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        coutf << "LJ" << "  " 
              << setprecision(17) << _particle(i).getposition(0,true)/_side(0) << "   " // x come precisione do tutte le cifre del double!
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x/2, 0)); //ho diviso per 2 per far partire la config da meta del lato per ogni direzione (da togliere)
      _particle(i).setposition(1, this->pbc(_side(1)*y/2, 1)); //sono ovunque unita di misure naturali, ma la configurazioee iniziale è sempre scritta in udm del lato, per questo moltiplico per la lunghezza del alto stesso.
...
//vettore per immagazzinare i bins dell'istogramma
    vec MaxwellBoltzmann(_n_bins_v); 
//inizializzo 
    MaxwellBoltzmann.zeros(); 
    //per ogni particella, misuro la velocità e la metto nel bin corrispondente
for (int i{};i<_npart;i++)
{
  //norma del vettore velocità della i esima particella
  
  double v=arma::norm(_particle(i).getvelocity()); 

//bin size= vmax/nbins, in questo momento la vmax è 4T

//trovo il bin in cui cade la velocità
int bin=(int)(v/_bin_size_v);
//controllo che la velocità sia minore di quella max
if(bin<_n_bins_v)
//memorizzo nell'istogramma
MaxwellBoltzmann(bin)+=1;
...
MaxwellBoltzmann/=(arma::sum(MaxwellBoltzmann)*_bin_size_v);
//accumulo i valori

//nel codice sono gia stati istanziati nbins indici liberi nel vettore measurement, che ora riempio creando nbins blocchi(uno per ogni bin)
for(int i{};i<_n_bins_v;i++)
{
  _measurement(_index_pofv+i)=MaxwellBoltzmann(i);
}
  ...
  if(_measure_magnet){
    ...
    for(int i=0;i<_npart;i++){
//sommo gli spin
      M+=_particle(i).getspin();
    }
//salvo
    _measurement(_index_magnet)=M/_npart;

  }

  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv){
    //qui calcolo solo H^2, perchè H lo calcola già nell'energia
      double H2=pow(_measurement(_index_tenergy),2);
      _measurement(_index_cv)=H2;
  }
  ...
  if(_measure_chi){
    ...
    //elevo e salvo
    _measurement(_index_chi)=pow(chi,2);
  }
...
if(_measure_pofv){ 
  ...
  coutf  <<setw(12)<<blk //blocco corrente
   <<setw(12)<<i*_bin_size_v //velocità centrale del bin corrente
          << setw(12) << sum_average/(double)blk //media sul blocco
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl; //errore
...
sum_ave2=_global_av2(_index_magnet);
 coutf<< setw(12) << _temp
          << setw(12) << blk //blocco corrente
          <<setw(12)<<average //M attuale
          << setw(12) << sum_average/(double)blk //media dei blocchi
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl; //errore
...
coutf.close();
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
   if(_measure_cv){
    ...
    coutf <<setw(12)<<_temp //temperatura
          << setw(12) << blk //blocco corrente
          << setw(12) << average //media di blocco
          << setw(12) << sum_average/double(blk)//media progressiva
          << setw(12) << this->error(sum_average, sum_ave2, blk) //errore
          << endl;
  ...
  if(_measure_chi){
    ...
    _global_av(_index_chi) -= _average(_index_chi); 
    _global_av2(_index_chi) -= _average(_index_chi) * _average(_index_chi);
    average = _beta*_average(_index_chi)/_npart;
    _global_av(_index_chi) += average;
    _global_av2(_index_chi) += average * average;
  ...
  coutf << setw(12)<<_temp << setw(12)<< fraction << endl;
  coutf.close();
