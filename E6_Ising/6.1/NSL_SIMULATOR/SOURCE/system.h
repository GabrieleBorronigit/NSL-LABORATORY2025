...
private:
  ...
  int _sim_type;        // Type of simulation (e.g., Lennard-Jones, Ising) //=0 per la dinamica molecolare
  ...
  field <Particle> _particle; // Field of particle objects representing the system //oggetto di armadillo che permette di creare vettori di oggetti a piacere, in queto caso un vettore di particlle
  vec _fx, _fy, _fz;    // Forces on particles along x, y, and z directions

  // Properties
  int _nprop; // Number of properties being measured //=5+N_bin (istogramma è un vetttore di proprietà)
  ...
  vec _block_av;         // Block averages of properties //accumulatore delle medie di blocco
  ...
  double _Cv_accum = 0.0;
    double _Cv2_accum = 0.0;
public: // Function declarations
  ...
  void reset_acc(){_global_av.zeros();
_global_av2.zeros();}
  void Reset_acceptance(){_nattempts=0;
  _naccepted=0;}
};
