  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = (8.0*M_PI*_rho)/(9.0*pow(_r_cut,9))-(8.0*M_PI*_rho)/(3*pow(_r_cut,3.0)); // TO BE FIXED IN EXERCISE 7
...
//normalizzo in averages
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    if(_sim_type==0){ //in Md l'energia cinetica la prendo dalla velocità delle particelle
    for (int i=0; i<_npart; i++) kenergy_temp += 0.5 * dot( _particle(i).getvelocity() , _particle(i).getvelocity() ); 
    kenergy_temp /= double(_npart);}
    else if(_sim_type==1){
      //in MC l'energia cinetica è fissata dalla temperatura di input (teo equipartizione dell'energia)
      kenergy_temp=0.5*_ndim*_temp;
    }
...
void System :: averages(int blk){
  ...
  for(int i = 0; i < _n_bins; i++){
     //normalizzo l'istogramma GOFR dividendo per la crosta sferica nello sp delle fasi
    double r1 = i * _bin_size;
    double r2 = (i+1) * _bin_size;
    double shell_volume = (4.0 * M_PI / 3.0) * (pow(r2, 3) - pow(r1, 3));
    double norm = _rho * _npart * shell_volume;
    double gofr_val = _average(_index_gofr + i) / norm;

    double global_val = _global_av(_index_gofr + i) / double(blk);
    double err = this->error(_global_av(_index_gofr + i), _global_av2(_index_gofr + i), blk) / norm;

    coutf << setw(12) << r1 //bin corrente
          << setw(12) << gofr_val //media di blocco
          << setw(12) << global_val / norm //media progressiva
          << setw(12) << err << endl; //errore
  }
  coutf<<endl; //separo i blocchi del ciclo
  coutf.close();
